...
init:  config (+ brownfield: survey the code → all-done tree → the fidelity gate)
  → plan #0:  explore → intent record (ref) + the whole tree as sketches
...
- **verify holds the yardstick against its source** (v0.9.5). Every gate after grow reads the Task as its criteria — implement builds to it, review tests against it — so a draft that silently *narrows* its ref intent is invisible to the whole rest of the chain: each later eye faithfully checks the narrowed text. Proof run #5 (Sonnet 4.6) shipped exactly this — the `today` node's draft dropped the ref's "a goal tag with no records today still shows," every gate passed on the narrowed contract, and the product's primary screen lost its habit view. So spec-verify's read now includes the ref clauses the node covers, and C1 maps them against the draft: each ref-stated behavior **carried, explicitly deferred, or a noted divergence** — silence or shrinkage is the finding. The check is bounded the way C5's Stop rule taught: only this node's clauses, `(gap)` spots excluded, presence-or-deferral only — never a demand that the AC measure each one. (grow already forbade silent narrowing; this is the catch-via-review twin that makes the rule hold for a literal executor.)
...
|---|---|---|---|
| Blank slate | just an idea | plan #0: explore → intent record → all-`sketch` tree | the record (produced at plan #0) |
| Rough spec | simple requirements | plan #0: explore fills gaps → intent record → all-`sketch` tree | the record (produced at plan #0) |
| Detailed spec | a detailed spec doc | plan #0: fill gaps → all-`sketch` tree | the spec doc |
