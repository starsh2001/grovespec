...
1. **Reopen** to the earliest status the change touches, with **`node .grovespec/bin/grovespec.mjs reopen TASK-N draft|approved`** — `draft` if the spec/contract changes (needs re-`verify`), `approved` if only the code changes (spec still valid → re-`implement` → re-`review`). The command resets the gate cycles so a past cycle's `passed` can never double as the new cycle's evidence (it keeps `reviewed_commit` as the next diff's base); **never reopen by hand-editing the status**. An **`approved` node reopens to `draft` only** — the spec gate is the one behind it, and a code-only change on a node with no code yet is `grovespec-implement`, not a revise.
...
**Then re-run the decomposition gate.** After the structure change (and any propagation), run `grovespec-verify` with the **explicit target `tree`** — auto-routing won't pick tree mode once a passed `tree.verify.yaml` exists, so name it.
- **Scale to the change**: a single move with the contract unchanged → `light`; a split·merge or several nodes → `standard`+ (round up).
- On a built tree the reviewers read each node's **id·name·Overview only** (`reviewers.md`), and `tree.verify.yaml`'s `adjudications` carry over as do-not-raise — settled calls aren't re-litigated.
...
**Commit the reopening as `revise TASK-N: <summary>`** before the re-run steps begin — the changed Task file(s), the tree edit if the structure moved, and the `reopen` result. The step name goes first (`FORMATS.md` "Commits"); the cycle that follows writes its own `TASK-N: ` commits.
