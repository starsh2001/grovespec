...
- `followups` holds real later revise/verify work (`kind: contract-gap`) or non-blocking notes (`kind: concern`) — and only entries whose gate1 is `behavior` or `mechanism`; a `contrived`/`story` keep is recorded as an adjudication instead (`validate` enforces — `reviewers.md` §triage).
...
| level | when | review |
|---|---|---|
| `skip` | trivial diff (format·comment·≤2 lines) · a propagation re-check with an empty diff (below) | tests only, no reviewers |
...
> **Propagation re-check.** A consumer reopened only by another node's contract change — no finding of its own — re-enters with one question: *does the changed contract reach this node's use of it?* Its implement pass enumerates this node's use-sites of the changed clauses/symbols in the Change Log. **No use-site reached and the code diff is empty → review runs at `skip`**: the self-check verifies that enumeration against the changed contract, and no cold wave spawns — an empty diff gives a cold lens nothing to read but the evidence prose, and grading prose is not reviewing code. Any reached use-site, or any code change → the normal scale rules. (Why the empty-diff path exists: METHODOLOGY §6.)
...
- **fix loop** — `grovespec-fix` applies the list; then ONE cold **confirmation round scoped to the fix diff + the items it claims closed + the in-node use-sites of what it changed** (not the whole diff again): item closed? new hole in what the fix touched? A serendipitous **out-of-scope** defect goes to `followups` through the mini-triage — it extends this cycle only with gate1 `behavior` or gate2 `yes` (`reviewers.md` §The cycle); the fix's own holes and regressions block as ever. **The list empty + the last confirmation clean → terminal pass.**
...
- **Clean terminal pass** (tests green + the cycle closed: ledger exhausted at find, confirmed list empty or closed, last confirmation clean) → **seal it**: `grovespec pin TASK-N` (reviewed commit + spec/Task digests, verdict pending; it refuses uncommitted reviewed project input outside this gate's Task/evidence); then run **`grovespec validate`** (must pass — a format/evidence violation blocks `done`) and **`grovespec fresh`**; show the human the result + test summary + any fresh signals; on **confirm** run **`grovespec approve TASK-N --human`** — it re-verifies the seal against the current committed and working bytes, including the pending Task frontmatter, and flips `status: done` itself; human confirmation replaces machine test evidence, not byte binding. **Never hand-edit the status.** The surviving `adjudications` stay in `<id>.review.yaml` — `reopen` preserves them across cycles, so a later cycle's cold rounds read them there instead of re-litigating. **Don't copy them into the Change Log**: the reviewers' criteria come from the Task file, and a Change Log carrying verdicts breaks the blankness it feeds.
